...
struct BST {
		...
		bool deepestNodes();
		void PrintDeep(int num,BTNode * cur);
		// sample operation (extra functions) - same as BT
		int countNode();		// count number of tree nodes
		bool findGrandsons(type);// find the grandsons of an input father item
		bool display(int, int);
		void ASCprintfile(BTNode*,ofstream &);
		void DESdisplay(BTNode*);
		void DESprintfile(BTNode* ,ofstream&);
		bool CloneSubtree(BST t1, type item);
		BTNode* CloneSubtree2(BTNode* cur, type item);
		bool printLevelNodes();
		bool printPath();
		void printexternalpath(BTNode* cur, int[], int);
